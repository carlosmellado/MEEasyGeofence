...
static const CLLocationDistance kOutOfAreaOffsetMeters = 100.0f; //Distance offset for geofences area radious reload region.
static const CLLocationDistance kMaxRadiusMeters = 5000.0f;
static const CLLocationDistance kMinRadiusMeters = 500.0f;
static const CLLocationDistance kGeotrackingRadius = 100.0f;
static NSString *const kOutOfAreaStringIdentifier = @"OUTOFAREA";
static NSString *const kGeotrackingIdentifier = @"kGeotrackingIdentifier";
...
- (void)clearCurrentGeotracking {
    ...
    for (CLCircularRegion *region in [[[[self class] sharedInstance] locationManager] monitoredRegions]) {
        ...
        if ([region.identifier isEqualToString:kGeotrackingIdentifier]) {
...
- (void)createRegionForGeotrackingWithLocation:(CLLocation *)location {
    ...
    [[self class] registerGeofenceWithCoordinates:location.coordinate radius:kGeotrackingRadius identifier:kGeotrackingIdentifier notifyOnEntry:NO notifyOnExit:YES];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations {
    ...
    for (CLCircularRegion *region in [[[self class] sharedInstance] locationManager].monitoredRegions) {
        ...
        if ([region.identifier isEqualToString:kGeotrackingIdentifier] || [region.identifier isEqualToString:kOutOfAreaStringIdentifier]) {
